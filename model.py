...
# 合并shape不一定一样的两个tensor
def mymerge(a, b, name=None):
    '''
    合并两个因为降采样再升采样后维度可能减一无法直接合并的两个4维tensor
    注意a，b有顺序关系，a的shape>=b
    :param name:
    :param a:
    :param b:
    :return:
    '''
    b = tf.pad(b, tf.constant([[0, 0], [0, 1], [0, 1], [0, 0]]), 'SYMMETRIC')
    b = tf.slice(b, [0, 0, 0, 0], tf.shape(a))
    return tf.concat([a, b], axis=3, name=name)
...
class UNET_sr():
    def __init__(sf, predict_flag=False, H=None, W=None):
        ...
        with sf.graph.as_default():
            if predict_flag:
                sf.x = tf.placeholder(tf.float32, [None, None, None, 1])
                # sf.istraining = True
                sf.prd = sf.gen(sf.x)
                return
            with tf.device('/cpu:0'):
